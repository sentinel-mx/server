 ...
 * Types Model
 */
class SentinelModelTypes extends JModelList
{
	...
	public function getItems()
	{
		...
		if (SentinelHelper::checkArray($items))
		{
			...
			foreach ($items as $nr => &$item)
			{
				...
				$access = ($user->authorise('type.access', 'com_sentinel.type.' . (int) $item->id) && $user->authorise('type.access', 'com_sentinel'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__sentinel_type'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__sentinel_type'))->set($fields)->where($conditions); 
