...
/**
 * Data_sets Table class
 */
class SentinelTableData_set extends JTable
{
	/**
	...
	{
		parent::__construct('#__sentinel_data_set', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_sentinel.data_set'));
	}	
	...
	/**
	 * Overload the store method for the Data_set table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New data_set. A data_set created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('data_set', 'SentinelTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_SENTINEL_DATA_SET_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_sentinel.data_set.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_sentinel.data_set.'.(int) $this->$k;
	}
